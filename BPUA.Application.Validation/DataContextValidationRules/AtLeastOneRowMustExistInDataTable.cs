...
namespace BPUA.Application.Validation
{
    ...
    public class AtLeastOneRowMustExistInDataTable : ValidationRule
    {
        ...
        public override bool Validate(IDataSet? dataContext)
        {
            ...
            {
                dataContext.AddMessage(MessageType.Error, $"{this.GetType().Name}_{DataTableName}", $"At least one row must exist in data table {DataTableName} at transition handler {RequestHandlerTypeFullName}");
                return false;
            }
